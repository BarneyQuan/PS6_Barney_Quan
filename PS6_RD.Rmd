...
**Answer: The table above is the polynomial in the standardized score of degree 0, and the coefficient above represents how these polynomials interacted with the eligibility indicator **



## **Question 15: Write down the specifications used in row 2 of columns 1,2 and 3 of table 1. **

**Answer: Columns 1–3 present specifications with different parameterizations of the 
functions f1(·) and f2(·): no polynomial, a first order polynomial (as in Figure 2 in the paper), and a quadratic polynomial, respectively. **
...
stargazer(reg4, reg5, reg6, reg7, reg8, reg9,
          ...
          omit = c("geographic_locality", "mean_age", "log_per_capita_income", "mean_household_size", "mean_education", "missing_mean_age", "missing_log_per_capita_income", "missing_mean_household_size", "missing_mean_education", "respondent_is_female", "respondent_age", "education_in_07", "missing_gender", "missing_age", "missing_education","predicted_income", "indicator*predicted_income", "indicator*predicted_income_sqrd"),
          dep.var.caption = c("Indicator (Standard Errors)"),
          dep.var.labels = c("Indicator (Standard Errors)"),
          covariate.labels = c("Government support, 2007 (during program)"), 
          notes.label = "Significance Levels",
          add.lines = list(c("Score controls", "None", "Linear", "Quadratic", "None", "Linear", "Quadratic"),c("Other controls", "No", "No","No","Yes", "Yes", "Yes")))

```

**Answer: Hold the other variables equal, **
...
**Answer:The bandwidth is used in how close to the threshold an observation’s running variable must be to be in the regression sample.**
...
**Answer: First off, the evidence in plot1 above that virtually all eligible households received the program while nearly all ineligible households did not, suggests that blatant patronage is unlikely to have occurred. Nonetheless, we present formal tests for nonrandom assignment around the eligibility threshold. First, we estimate equation (1) for multiple baseline covariates measured in 2005 in the table above (and present the results graphically in Figure A1 in
the online Appendix). If eligibility score manipulation occurred systematically, we might find these characteristics varying discontinuously at the eligibility threshold, to the extent that they are correlated with households’ political orientation. **
 

## **Question 22: Was this threshold eligibility score specifically designed for this particular program? Why does this matter?**

**Answer: Yes, the true eligibility threshold provides the best fit to the data as measured by the regression R2 (not shown), providing reas-surance that the discontinuity we exploit is a genuine feature of the data. As an additional robustness check, we take 
advantage of the fact that the PANES eligibility threshold differs slightly across Uruguayan regions to estimate a difference-in-differences model, conditioning on the unstandardized income score and regional fixed effects, and focusing on the coefficient estimate on an indicator for PANES eligibility in that region. ** 
